...
const Menu = () => {
  ...
  return (
    <div className="relative">
      <button onClick={toggleDrawer} className={ `text-2xl text-white block mb-1 hover:text-yellow-400 absolute top-4 left-4 z-20 ${isOpen ? 'hidden' : ''}`}>
        <FontAwesomeIcon icon={faBars} />
      </button>

      {/* Button with times icon to close the drawer */}
      <button onClick={closeDrawer} className={`text-white block mb-1 hover:text-yellow-400 absolute top-4 left-4 z-20 ${isOpen ? '' : 'hidden'}`}>
        ...
        <div className="p-6 drawer-content" onClick={handleDrawerClick}>
          {menu.map((category) => (
            <div key={category.id} className="mb-6">
              <div className="text-white font-bold text-lg mb-3">{category.title.toUpperCase()}</div>
